...
deff('y=myf(A,B,C,D,x)',['y=A*x^2+B*x+C']);  // função de terceiro grau
deff('dy=dyf(A,B,C,D,x)',['dy=2*A*x+ B']); // função derivada

h0=gcf();
h0.figure_size= [480,700];
h0.figure_position = [45,68];
h0.figure_name="UENF - Valores dos coeficientes de "+"Ax2+Bx+C";
...
hg=figure();
hg.figure_name = "Grafico de uma função do Segundo Grau";
...
xtitle('Gráfico de uma função do segundo grau');   // titulo do grafico
